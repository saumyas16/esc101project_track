...
$cg=$_SESSION['tcoins']-$_SESSION['cut'];
$rn=$_SESSION['id'];
				$sqll="UPDATE olduserdetails SET tcoins='$cg' WHERE id='$rn'";
				$_SESSION['tcoins']=$cg;
				$results = $conn->query($sqll) or die("Error: ". mysql_error(). " with query ");
				$kp=$_SESSION['chlnaccp']+1;
				$sqll1="UPDATE olduserdetails SET chlnaccp='$kp' WHERE id='$rn'";
				$_SESSION['chlnaccp']=$kp;
				$results1 = $conn->query($sqll1) or die("Error: ". mysql_error(). " with query ");
				$kpi=$_SESSION['uaccp1']+1;
				$rc=$_SESSION['id2'];
				$sqll1="UPDATE olduserdetails SET uaccp='$kpi' WHERE id='$rc'";
				$_SESSION['uaccp1']=$kpi;
				$results1 = $conn->query($sqll1) or die("Error: ". mysql_error(). " with query ");
...
if($result->num_rows > 0) {
	...
	{
if($row['questitle']==$_SESSION['chin'])
{
            $a=$row["wincoin"];
            $b=(5*$a)/100;
			echo "<font size=\"14px\">";
